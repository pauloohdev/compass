...
<section id="landingpage" className="bg-[#F5F4F0] py-24 lg:py-28">
  <div className="mx-auto max-w-6xl px-6 lg:px-8">
    ...
    <div className="mt-14 grid gap-6 md:grid-cols-3 md:gap-12">
      {plans.map((plan, i) => (
        <div
          key={plan.title}
          onMouseEnter={() => setHovered(i)}
          onMouseLeave={() => setHovered(null)}
          className={`relative overflow-hidden transition-colors duration-300 px-10 py-16 md:px-14 md:py-20 ${i === 1 ? 'ring-2 ring-[#BA7517] ring-opacity-50 shadow-lg shadow-[#BA7517]/20' : ''}`}
          style={{
            backgroundColor: hovered === i ? "#111110" : "transparent",
            ...(i === 1 && !hovered ? { boxShadow: '0 0 20px rgba(186, 117, 23, 0.3)' } : {}),
          }}
        >
          ...
          <button className={`w-full flex items-center justify-center gap-2 text-sm font-medium transition-all border rounded-full py-3 px-6 hover:bg-[#BA7517] hover:text-[#111110] hover:border-[#BA7517] ${hovered === i ? "border-[#f5f4f033] text-[#f5f4f080]" : "border-[#11111040] text-[#11111066]"}`}>
